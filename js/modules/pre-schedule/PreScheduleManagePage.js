...
export class PreScheduleManagePage {
    ...
    async render() {
        ...
        // 省略了 settingsModalHtml 和 searchModalHtml 的重複部分，請使用之前的版本
        // 重點在下面的 reviewModalHtml
        ...
        // 為了完整性，這裡還是需要 settingsModalHtml 和 searchModalHtml 
        // (請複製上一則回覆的完整 HTML 字串，這裡省略以避免字數過多)
    ...
    async afterRender() {
        ...
        document.getElementById('btn-add').addEventListener('click', () => { 
            // 這裡需要重新填充 modal 內容，因為上面用了 placeholder
            document.getElementById('pre-form-content-placeholder').innerHTML = this.getPreFormHtml(); // 呼叫 helper
            this.openModal(null);
        });
        ...
        document.addEventListener('click', (e) => { if(!e.target.closest('#shift-context-menu')) document.getElementById('shift-context-menu').style.display = 'none'; });
    ...
    async loadList(uid) { /* 同前 */ if(!uid) return; this.targetUnitId = uid; const tbody = document.getElementById('table-body'); this.preSchedules = await PreScheduleService.getPreSchedulesList(uid); tbody.innerHTML = this.preSchedules.map(p => `<tr><td>${p.year}-${p.month}</td><td>${p.settings.openDate}~${p.settings.closeDate}</td><td>${p.staffIds.length}</td><td>${p.status}</td><td class="text-end"><button class="btn btn-sm btn-success me-1" onclick="window.routerPage.openReview('${p.id}')">審核</button><button class="btn btn-sm btn-outline-primary me-1" onclick="window.routerPage.openModal(null)">設定</button><button class="btn btn-sm btn-outline-danger" onclick="window.routerPage.deletePreSchedule('${p.id}')">刪</button></td></tr>`).join(''); }
    ...
    // Modal Helper logic (Open, RenderInputs, AddStaff, etc. 同前，請確保存在)
    async openModal(idx) {
        ...
        // Render Logic for Edit/New... (同前一版)
        if(idx !== null) { /* Edit Logic */ } else { /* New Logic */ }
        this.renderStaffList([]);
        this.modal.show();
    }
    
    // ... renderGroupInputs, renderStaffList, searchStaff, addStaffFromSearch, savePreSchedule, deletePreSchedule ...
    // (這些輔助方法請直接沿用上一版的內容，為了不讓回應過長被截斷，請確保這部分已整合)
