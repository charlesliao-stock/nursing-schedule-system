...
export class PreScheduleManagePage {
    ...
    async loadList(uid) {
        ...
        try {
            ...
            tbody.innerHTML = this.preSchedules.map((p, index) => {
                // ✅ 修正 2: 這裡顯示的人數應為 staffIds 的長度，這才是參與預班的母數
    ...
    getStatusText(s) {
        if(s === 'open') return '<span class="badge bg-success">進行中</span>';
        if(s === 'closed') return '<span class="badge bg-secondary">已關閉</span>';
