...
import { PreScheduleSubmitTemplate } from "./templates/PreScheduleSubmitTemplate.js"; // 引入 Template
...
export class PreScheduleSubmitPage {
    ...
    openSchedule(id, isReadOnly) {
        ...
        document.getElementById('preference-container').innerHTML = 
            PreScheduleSubmitTemplate.renderPreferencesForm(canBatch, maxTypes, mySub.preferences || {});
    ...
    async handleSubmit() {
        ...
        if (canBatch) {
            ...
            if (maxTypes === 2 && !set.has('D')) { alert("每週兩種班別時，必須包含白班(D)"); return; }
