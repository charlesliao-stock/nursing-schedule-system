...
export class ShiftSettingsPage {
    ...
    renderTable() {
        const tbody = document.getElementById('table-body');
        if(this.shifts.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="text-center py-5 text-muted">無班別</td></tr>'; return; }
        tbody.innerHTML = this.shifts.map((s, i) => `
            <tr>
                <td class="fw-bold" style="color:${s.color}">${s.code}</td><td>${s.name}</td><td>${s.startTime} ~ ${s.endTime}</td>
                <td><span class="badge rounded-pill border" style="background:${s.color}; width:24px; height:24px;">&nbsp;</span></td>
                <td class="text-end pe-3"><button class="btn btn-sm btn-outline-primary me-1" onclick="window.routerPage.openModal(${i})"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-outline-danger" onclick="window.routerPage.deleteShift(${i})"><i class="fas fa-trash"></i></button></td>
            </tr>`).join('');
    ...
    async saveShift() {
        ...
        this.modal.hide(); this.renderTable();
    }
    
    async deleteShift(idx) { if(confirm('刪除？')) { this.shifts.splice(idx, 1); await UnitService.updateUnit(this.targetUnitId, { "settings.shifts": this.shifts }); this.renderTable(); } }
}
