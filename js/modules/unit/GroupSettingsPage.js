...
export class GroupSettingsPage {
            ...
            <div class="container-fluid mt-4">
                ...
                <div class="card shadow-sm mb-4 border-left-primary">
                    <div class="card-body py-2 d-flex align-items-center gap-2">
                        ...
                        <button id="btn-add" class="btn btn-primary w-auto text-nowrap"><i class="fas fa-plus"></i> 新增組別</button>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="card shadow mb-4">
                            ...
                            <ul class="list-group list-group-flush" id="group-list"><li class="list-group-item text-center text-muted py-4">載入中...</li></ul>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="card shadow">
                            ...
                            <div class="card-body p-0 table-responsive" style="max-height: 600px;">
                                <table class="table table-hover mb-0 align-middle"><thead class="table-light sticky-top"><tr><th>姓名</th><th>職級</th><th>組別</th></tr></thead><tbody id="staff-tbody"></tbody></table>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    renderStaff() {
        ...
        tbody.innerHTML = this.staffList.map(u => `<tr><td class="fw-bold">${u.name}</td><td><span class="badge bg-light text-dark border">${u.rank}</span></td><td><select class="form-select form-select-sm group-select" data-uid="${u.uid}" style="width:120px;">${opts.replace(`value="${u.group}"`, `value="${u.group}" selected`)}</select></td></tr>`).join('');
    }
    ...
    async deleteGroup(idx) { if(confirm('刪除？')) { this.groups.splice(idx, 1); await UnitService.updateUnit(this.targetUnitId, { groups: this.groups }); this.renderGroups(); this.renderStaff(); } }
    ...
    async saveAssignments() {
        ...
        document.querySelectorAll('.group-select').forEach(sel => {
            ...
            if((original.group || '') !== val) { updates.push(userService.updateUser(uid, { group: val })); original.group = val; }
        });
