export const SwapApplyTemplate = {
    // 1. 主画面佈局
    renderLayout() {
        const reasons = ['单位人力调整', '公假', '病假', '丧假', '支援', '个人因素', '其他'];
        const reasonOptions = reasons.map(r => `<option value="${r}">${r}</option>`).join('');

        return `
            <div class="container-fluid mt-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h3><i class="fas fa-exchange-alt text-primary me-2"></i>申请换班 (多笔)</h3>
                </div>

                <div id="admin-impersonate-section" class="card shadow-sm mb-4 border-left-warning" style="display:none; background-color: #fffbf0;">
                    <div class="card-body py-2 d-flex align-items-center justify-content-between flex-wrap gap-2">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <strong class="text-dark">管理者模拟：</strong>
                            ...
                            <button id="btn-impersonate" class="btn btn-warning btn-sm text-dark fw-bold">
                                <i class="fas fa-mask"></i> 切换身份
                            </button>
                        </div>
                        
                        <div id="impersonation-status" class="alert alert-warning mb-0 py-1 px-3 small border-warning text-dark fw-bold" style="display:none;">
                            正在模拟：<span id="current-impersonating-name" class="text-primary fs-6"></span>
                            <button id="btn-exit-impersonate" class="btn btn-outline-dark btn-sm ms-3 py-0" style="font-size: 0.8rem;">
                                退出模拟
                            </button>
                        </div>
                    </div>
                            ...
                            <div class="card shadow mb-3">
                                <div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
                                    <div class="small text-muted">
                                        ...
                                        操作：1.点选<span class="badge bg-primary">您的班</span> 2.点选该日<span class="badge bg-success">对方的班</span> (限同日互换)
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-4">
                            <div class="card shadow border-left-primary h-100">
                                <div class="card-header bg-primary text-white fw-bold d-flex justify-content-between">
                                    <span>换班申请单</span>
                                    <span class="badge bg-white text-primary" id="swap-count-badge">0 笔</span>
                                </div>
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label small fw-bold">换班理由 (必填)</label>
                                        <select id="swap-reason-select" class="form-select form-select-sm mb-2">
                                            ${reasonOptions}
                                        </select>
                                        <input type="text" id="swap-reason-text" class="form-control form-control-sm" placeholder="请输入其他理由..." style="display:none;">
                                    </div>
                ...
                <div class="card shadow mt-4">
                    <div class="card-header py-3 bg-white"><h6 class="m-0 font-weight-bold text-secondary">我的申请纪录</h6></div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0 align-middle">
                                <thead class="table-light"><tr><th>日期</th><th>对象</th><th>内容</th><th>理由</th><th>状态</th></tr></thead>
    ...
    renderMatrix(schedule, staffList, currentUser, year, month) {
        ...
        html += `<thead class="table-light sticky-top"><tr><th style="min-width:80px; position:sticky; left:0; z-index:10; background:#f8f9fa;">人员</th>`;
        ...
        return items.map((item, idx) => `
            <li class="list-group-item position-relative bg-white mb-2 border rounded shadow-sm">
                <button class="btn btn-sm btn-outline-danger border-0 position-absolute top-0 end-0 m-1" 
                        onclick="window.routerPage.removeSwapFromList(${idx})" title="删除此笔">
    ...
    renderHistoryRows(requests) {
        if (!requests || requests.length === 0) return '<tr><td colspan="5" class="text-center text-muted p-4">无申请纪录</td></tr>';
